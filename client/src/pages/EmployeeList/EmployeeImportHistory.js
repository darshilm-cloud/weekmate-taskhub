...
const EmployeeImportHistory = ({ visible, onClose, onImportComplete }) => {
  ...
  const fetchHistory = useCallback(async (page = pagination.current, silent = false) => {
    ...
    try {
      ...
      const response = await Service.makeAPICall({
        ...
        api_url: `${Service.importHistory}?${params.toString()}`,
      });
  ...
  const handleCancel = async (jobId) => {
    ...
    try {
      const response = await Service.makeAPICall({
        ...
        api_url: `${Service.importCancel}/${jobId}`,
      });
  ...
  const handleUndo = async (jobId) => {
    ...
    try {
      const response = await Service.makeAPICall({
        methodName: Service.postMethod,
        api_url: `${Service.importUndo}/${jobId}`,
      });
  ...
  // ── Download error CSV — streams through auth'd backend endpoint ─────────
  const handleDownloadErrorCsv = async (jobId) => {
    setActionLoading((prev) => ({ ...prev, [jobId]: "download" }));
    try {
      const accessToken = localStorage.getItem("accessToken");
      const apiUrl = `${Service.API_URL}${Service.importErrorCsv}/${jobId}`;
        ...
        return (
          <Space size={4}>
            ...
            {IN_PROGRESS_STATUSES.has(record.status) && (
              <Popconfirm
                title="Cancel this import? Partially imported employees will be removed."
            ...
            {canUndo(record) && (
              <Popconfirm
                title="Undo this import? All imported employees will be permanently deleted."
    ...
    <Modal
      title={
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span>Employee Import History</span>
...
export default EmployeeImportHistory;
