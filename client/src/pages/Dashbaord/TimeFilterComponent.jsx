...
import dayjs from "dayjs"; // Add dayjs import for date manipulation
...
const TimeFilterComponent = ({ onFilterChange }) => {
  ...
  const [selectedFilters, setSelectedFilters] = useState({
    [FILTER_TYPES.PROJECT]: [],
    [FILTER_TYPES.DATE]: currentMonthRange, // Set current month as default
  });
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    // Check if date filter is different from current month (default)
    const currentMonth = getCurrentMonthRange();
    const isDateFilterActive = selectedFilters.date.startDate && selectedFilters.date.endDate && 
      !(selectedFilters.date.startDate.isSame(currentMonth.startDate, 'day') && 
        selectedFilters.date.endDate.isSame(currentMonth.endDate, 'day'));
  ...
  const resetFilter = useCallback(
    (filterType) => {
      if (filterType === FILTER_TYPES.PROJECT) {
        ...
        // Reset to current month range instead of null values
        setSelectedFilters((prev) => ({
          ...prev,
          [filterType]: getCurrentMonthRange(),
        }));
      }
  ...
  const resetAllFilters = useCallback(() => {
    setSelectedFilters({
      [FILTER_TYPES.PROJECT]: [],
      [FILTER_TYPES.DATE]: getCurrentMonthRange(), // Reset to current month range
    });
  ...
  const renderFilterContent = () => {
    ...
    return (
      <FilterSection
        config={config}
        items={config.api ? filterData[activeFilter] : []}
  ...
  const hasActiveFilter = (key, value) => {
    // Special case for NEED_TO_BILL_CUSTOMER
    if (key === FILTER_TYPES.NEED_TO_BILL_CUSTOMER) {
      return value !== "All";
    }

    // Special case for DATE filter - only show as active if different from current month
    if (key === FILTER_TYPES.DATE) {
      if (!value || !value.startDate || !value.endDate) return false;
      
      const currentMonth = getCurrentMonthRange();
      return !(value.startDate.isSame(currentMonth.startDate, 'day') && 
               value.endDate.isSame(currentMonth.endDate, 'day'));
    }
    ...
    // If value is an object (e.g., date range) - for other object types
      ...
      <div className="filter-sidebar">
        ...
        {createFilterMenuItems().map((item) => (
          ...
          >
            ...
            {hasActiveFilter(item.key, selectedFilters[item.key]) && (
