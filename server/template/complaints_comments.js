...
class CompalaintComplaintsMail {
  newComplaintCommentsMail = async (data, companyId) => {
    try {
        ...
        data?.attachments && data?.attachments?.length > 0
          ? `
            <tr>
                ...
                <td style="padding: 10px; border: 1px solid #ddd; width: 70%;"><a href ="${
                  process.env.REACT_URL + "public/" + data?.attachments[0].path
                }" style="color: #187cb7; text-decoration: underline;"> File </a> </td>
            </tr>
            `
          : "";
      let html = `
            <div style="font-family: Arial, sans-serif; color: #333;">
                ...
                <h3 style="color: #187cb7;">New Comment Added</h3>
                    ...
                    <p style="margin: 0;">${
                      companyData?.companyName || "Taskhub"
                    }</p>
                </div>
            </div>
            `;
      ...
      const mailData = {
        subject: `New Complaint received by ${
          data?.complaints?.client_name
        } - ${data?.technology?.project_tech} - ${
          data?.complaints?.priority.charAt(0).toUpperCase() +
          data?.complaints?.priority.slice(1).toLowerCase()
        }.`,
        html
      };
