...
<html lang="en">
  ...
  <body>
          ...
          <tbody>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td valign="meddile" style="color: #333; font-weight: 200; font-size: 16px">
                        <b>${getUserName(
                          data?.createdBy
                        )}</b> has <b>mentioned</b> you in the topic comment:
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <tr>
              <td style="
                ...
                <b>${getUserName(
                  data?.createdBy
                )}</b> has <b>mentioned</b> you in the topic comment: <b>${
      data?.title
    } 
                </b>
                <br />
    ...
    // to get mailSettings of taggedUsers as of sending mail after 4 hours setting.
    let quarterlymailSettingsData = mailSettings.filter(
      (ele) => ele.quarterlyMail
    );

    // to get that taggedUsers mailids whose mail setting for discussion_tagged is true
    let mailIds = data?.taggedUsers
      .filter(
        (s) =>
          s && mailSettingsData?.some((mail) => mail.createdBy.equals(s._id))
      )
      .map((tagged) => tagged.email);

    //to get that taggedUsers mailids whose mail setting for quarterlyMail is true
    let quarterlymailIds = data?.taggedUsers
      .filter(
        (s) =>
          s &&
    ...
    console.log("template/mail.js error at 293 ~> ", e);
  }
};
