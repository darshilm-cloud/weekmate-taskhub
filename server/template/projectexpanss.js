...
class ProjectExpansseMail {
  newProjectExpecesMail = async (data,lognusername) => {
    ...
    try {
      ...
      // console.log(data, 'DataTemp',lognusername);
...
<div style="font-family: 'Arial', sans-serif; color: #333; line-height: 1.6; max-width: 800px; margin: 0 auto; background-color: #f4f4f8; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
  ...
  <div style="background-color: #03497a; color: #fff; padding: 20px; text-align: center;">
    <h1 style="margin: 0; font-size: 20px;"> Project Expense Request for ${data?.project?.title} </h1>
  </div>
  ...
  <div style="padding: 20px; background-color: #fff;">
    ...
    <p style="margin: 0 0 15px; font-size: 16px;">
      <strong>${data?.createdBy?.full_name?data?.createdBy?.full_name:lognusername?.full_name}</strong> has requested a new purchase for the project <strong>${data?.project?.title}</strong>.
      Please review the request and approve/reject the request <span><a href=${process.env.REACT_URL}edit/projectexpenseform/${data?._id}>here</a></span>.
    </p>
    ...
    <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
      <tr style="background-color: #f9f9f9;">
        ...
        <td style="padding: 12px; border: 1px solid #eaeaea;">${data?.purchase_request_details}</td>
      </tr>
      <tr>
        <td style="padding: 12px; border: 1px solid #eaeaea; font-weight: bold;">Cost</td>
        <td style="padding: 12px; border: 1px solid #eaeaea;">$${data?.cost_in_usd}</td>
      </tr>
      <tr style="background-color: #f9f9f9;">
        <td style="padding: 12px; border: 1px solid #eaeaea; font-weight: bold;">Need to Bill Customer?</td>
        <td style="padding: 12px; border: 1px solid #eaeaea;">${data?.need_to_bill_customer ? "Yes" : "No"}</td>
      </tr>
${data?.is_recuring ? `
<tr style="background-color: #f9f9f9;">
...
`;
      ...
      const mailData = {
        subject: `Expense Request for  ${data?.project?.title}  ~  ${' '+data?._id}`,
        html,
      };
      ...
      let to = [process.env.CEO_EMAIL,
      process.env.DIRECTOR_EMAIL]



      await emailSenderForPMS(to, mailData, cc);
    } catch (error) {
      console.log("🚀 ~ ComplaintMail ~ newComplaintMail= ~ error:", error);
    }
  };

  approveProjectExpecesMail = async (data, updatedName) => {
    try {
      // console.log(data,'dataMail',process.env.CEO_EMAIL,
      //   process.env.DIRECTOR_EMAIL,
      //   data?.manager?.email);
...
<div style="font-family: 'Arial', sans-serif; color: #333; line-height: 1.6; max-width: 800px; margin: 0 auto; background-color: #f4f4f8; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
  ...
  <!-- Header Section -->
  <div style="background-color: #03497a; color: #fff; padding: 20px; text-align: center;">
    <h1 style="margin: 0; font-size: 20px;">Approved - Project Expense Request for ${data?.project?.title} </h1>
  </div>
  ...
  <div style="padding: 20px; background-color: #fff;">
    ...
    <p style="margin: 0 0 15px; font-size: 16px;">
      <strong>${updatedName?.full_name}</strong> has approved the purchase for the project 
      <strong>${data?.project?.title}</strong>. Please proceed with the purchase and update the status along with the invoice <span><a href=${process.env.REACT_URL}edit/projectexpenseform/${data?._id}>here</a></span>.
    </p>
...
`;
      ...
      let to = [process.env.ACCOUNTANT_EMAIL_ID,
  ...
  paidProjectExpecesMail = async (data) => {
    try {
      // console.log(data,'dataMail',process.env.CEO_EMAIL,
      //   process.env.DIRECTOR_EMAIL,
      //   data?.manager?.email);
...
<div style="font-family: 'Arial', sans-serif; color: #333; line-height: 1.6; max-width: 800px; margin: 0 auto; background-color: #f4f4f8; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
  ...
  <!-- Header Section -->
  <div style="background-color: #03497a; color: #fff; padding: 20px; text-align: center;">
    <h1 style="margin: 0; font-size: 22px;">Paid - Project Expense Request for ${data?.project?.title} </h1>
  </div>

  <!-- Email Body -->
  <div style="padding: 20px; background-color: #fff;">
    <p style="margin: 0 0 15px; font-size: 16px;">Dear <strong>${data?.createdBy?.full_name}</strong>,</p>
