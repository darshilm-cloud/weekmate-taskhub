...
const ProjectTaskModel = mongoose.model("projecttasks");
...
exports.addComment = async (req, res, next) => {
  try {
    ...
    let data = new CommentsModel({
      ...
      task_id: value.subTask_id ? null : value.task_id,
      subTask_id: value.task_id ? null : value.subTask_id,
    ...
    if (value?.attachments && value.attachments.length > 0) {
      // get project id..
      const details = value.task_id
        ? await ProjectTaskModel.findById(
            new mongoose.Types.ObjectId(value.task_id)
          )
        : await ProjectSubTaskModel.findById(
            new mongoose.Types.ObjectId(value.subTask_id)
          );
...
exports.CommentsList = async (req, res, next) => {
  try {
    ...
    const andFilter = {};
    if (value.task_id) {
      matchQuery.task_id = new mongoose.Types.ObjectId(value.task_id);
      matchQuery.subTask_id = null;
    } else if (value.subTask_id) {
      matchQuery.subTask_id = new mongoose.Types.ObjectId(value.subTask_id);
      matchQuery.task_id = null;
...
exports.editComment = async (req, res, next) => {
  try {
    ...
    const details = updatedData.task_id
      ? await ProjectTaskModel.findById(
          new mongoose.Types.ObjectId(updatedData.task_id)
        )
      : await ProjectSubTaskModel.findById(
          new mongoose.Types.ObjectId(updatedData.subTask_id)
        );
