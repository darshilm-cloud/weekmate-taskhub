...
const ClientImportQueueService = require('../jobs/ClientImportQueueService');
...
// Start a new background client import
exports.startImport = async (req, res) => {
  try {
    ...
    const response = await ClientImportQueueService.processBulkClientImport(
      req.file.buffer,
    ...
    console.error('clientImport startImport error:', err);
...
exports.getAllImportHistory = async (req, res) => {
  try {
    ...
    return successResponse(res, statusCode.SUCCESS, 'Client import history fetched', records, {
      total,
      page,
      limit,
...
exports.getImportProgress = async (req, res) => {
  try {
    ...
    const jobStatus = await ClientImportQueueService.getJobStatus(jobId);
...
exports.cancelImport = async (req, res) => {
  try {
    ...
    const response = await ClientImportQueueService.cancelImport(jobId, userId);
    return res.status(response.status).json(response.data);
  } catch (err) {
    return catchBlockErrorResponse(res, err.message);
  }
...
exports.undoImport = async (req, res) => {
  try {
    ...
    const response = await ClientImportQueueService.undoImport(jobId, userId);
    return res.status(response.status).json(response.data);
  } catch (err) {
    return catchBlockErrorResponse(res, err.message);
  }
...
// Download the error CSV for a completed import
exports.downloadErrorCsv = async (req, res) => {
  try {
    ...
    const fileName = `client_errors_${jobId}.csv`;
