...
exports.addComplaintStatus = async (req, res) => {
    try {
        ...
        await data.save();
        const dataAtCompalint=await Complaints.findByIdAndUpdate({_id:value?.complaint_id},{
            status: value?.status || null,
        })
        ...
        if (value?.resolved_status != undefined && value?.resolved_status) {
            let emailDetails = await this.getComplaintStatusDetailsForMail(data._id);
            await newComplaintStatusResolutionFeedbackMailToClient(emailDetails)
        }
    ...
    try {
        ...
        if (value?.search) {
            matchQuery = {
                ...matchQuery,
                ...searchDataArr(
                    ["root_cause", "corrective_action", "immediate_action", ...(!value.isSearch ? ["manager.full_name", "acc_manager.full_name", "project.title", "complaints.complaint"] : []),],
                    value?.search
                ),
            };
        }
...
exports.getComplaintStatusDetailsForMail = async (
    statusId,
) => {
    try {
        const mainQuery = [
